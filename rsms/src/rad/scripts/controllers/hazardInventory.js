...
angular.module('HazardInventory', [
    ...
    .filter('everyThirdChecked', function () {
        return function (hazards) {
            ...
            for (var i = 0; i < hazards.length; i++) {
                //fizz buzz it for fun and inside joke fake randomness
                if (i % 3 == 0 || i % 5 == 0 && !hazards[i].unchecked) {
                    hazards[i].checked = true;
                }
                if (i != 0 && i % 3 == 0 && i % 5 == 0) {
                    hazards[i].checked = false
                }
                if(hazards[i].unchecked)hazards[i].checked = false;
            }
            return hazards;
        }
    })
    .controller('HazardInventoryCtrl', function ($scope, $q, $http, actionFunctionsFactory, $modal) {
        ...
        var getAllPIs = function () {
                return actionFunctionsFactory.getPI(1)
                    .then(
                        function () {
                            $scope.PI = dataStoreManager.getById("PrincipalInvestigator", 1);
                            $scope.buildings = $scope.PI.Buildings;
                        },
                        function () {
                ...
                return actionFunctionsFactory
                    ...
                    .then(
                        ...
                        function (hazards) {
                            ...
                            hazard.loadSubHazards().then(
                                function (subs) {
                                    $scope.hazards = hazard.ActiveSubHazards
                                }
                            );
                        },
                        function () {
        ...
        $scope.handleHazardChecked = function(hazard){
            console.log(hazard);
