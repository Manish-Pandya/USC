    ...
    module.directive('sticky', ['$window', '$timeout', function ($window, $timeout) {
        return {
            ...
            link: function linkFn($scope, $elem, $attrs) {
                ...
                var stickyLine;
                var initialCSS;
                var globalWidth = $elem[0].offsetWidth - 50;
                ...
                var positionObj;
                function getPos() {
                    if ($elem.hasClass("submit-cap")) {
                        if (!positionObj) {
                            var infoDiv = $body.find(".inspectionReportTable");
                            //console.log(infoDiv[0].getBoundingClientRect());
                            positionObj = infoDiv[0].getBoundingClientRect();
                            console.log(infoDiv.position());
                        }
                        $elem.css({
                            top: infoDiv.position().top-30 + 'px'
                        })
                    }
                };
                getPos();
                ...
                function stickElement(closestLine) {
                    ...
                    isSticking = true;
                    $timeout(function () {
                        initialCSS.offsetWidth = globalWidth;
                    }, 0);
                ...
                var onChange = function (newVal, oldVal) {
                    ...
                    if (valChange && notSticking && newVal > 0 && elemIsShowed) {
                        ...
                        if (elemIsShowed && elemWasHidden) {
                            $scope.updateStickyContentUpdateDimensions(globalWidth, $elem[0].offsetHeight);
                        }
                ...
                $scope.updateStickyContentUpdateDimensions = function (width, height) {
                    if (width && height) {
                        initSticky();
                        initialCSS.width = globalWidth + 'px';
