...
class HazardChangeManager {
    ...
    public function __construct(){
        ...
        $this->meta->table = 'hazard';
        $this->meta->hazard = new Hazard();
        $this->meta->f_id = Field::create('key_id', $this->meta->table);
        $this->meta->f_name = Field::create('name', $this->meta->table);
        $this->meta->f_parent_hazard = Field::create('parent_hazard_id', $this->meta->table);

        // Processors
        $this->processors = array(
            AddAction::ADD => new AddActionProcessor( $this->appActionManager, $this->meta ),
            MoveAction::MOVE => new MoveActionProcessor( $this->appActionManager, $this->meta ),
            InactivateAction::INACTIVATE => new InactivateActionProcessor( $this->appActionManager, $this->meta ),
            DeleteAction::DELETE => new DeleteActionProcessor( $this->appActionManager, $this->meta ),
            RenameAction::RENAME => new RenameActionProcessor( $this->appActionManager, $this->meta ),

            ReorderAction::REORDER => new ReorderActionProcessor( $this->appActionManager, $this->meta),

            MergeAction::MERGE => new MergeActionProcessor($this->appActionManager, $this->meta),
            NoteAction::NOTE => new NoteActionProcessor($this->appActionManager, $this->meta)
        );
    }
    ...
    public function do_action( A_HazardChangeAction &$action ){
        $LOG = LogUtil::get_logger(TASK_NUM, __CLASS__, __FUNCTION__);
