  ...
  (CASE
    ...
    WHEN insp.notification_date IS NOT NULL THEN
      CASE
        ...
        WHEN (
              (SELECT count(*) = 0 FROM deficiency_selection    WHERE response_id in (SELECT key_id FROM `response` WHERE inspection_id = insp.key_id))
          AND (SELECT count(*) = 0 FROM supplemental_deficiency WHERE response_id in (SELECT key_id FROM `response` WHERE inspection_id = insp.key_id))
        ) THEN 'CLOSED OUT'
